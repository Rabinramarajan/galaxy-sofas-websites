  ...
  return (
    <>
      ...
      <section className="py-20">
        <Container>
          <Reveal>
            <p className="text-[11px] uppercase tracking-[0.28em] text-walnut">The showroom</p>
            <h2 className="mt-3 max-w-2xl font-display text-4xl sm:text-5xl">Three rooms. One way of furnishing a home.</h2>
          </Reveal>
          <div className="mt-12">
      ...
      <section className="bg-linen py-20">
        <Container>
          <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
            <div>
              <p className="text-[11px] uppercase tracking-[0.28em] text-walnut">Featured pieces</p>
              <h2 className="mt-3 font-display text-4xl">Currently on the floor</h2>
            </div>
      ...
      <section className="py-20">
        <Container className="grid items-center gap-12 lg:grid-cols-2">
          ...
          <Reveal>
            <p className="text-[11px] uppercase tracking-[0.28em] text-walnut">How we work</p>
            <h2 className="mt-3 font-display text-4xl">Materials you can sit with for years</h2>
      ...
      <section className="bg-charcoal py-20 text-parchment">
        <Container>
          <p className="text-[11px] uppercase tracking-[0.28em] text-sand">Collections</p>
          <h2 className="mt-3 font-display text-4xl">Rooms, not isolated products</h2>
          <div className="mt-10 grid gap-6 md:grid-cols-2">
            {collections.map((collection) => (
              <Link key={collection.slug} href={`/collections#${collection.slug}`} className="group relative min-h-64 overflow-hidden">
                <SafeImage
      ...
      <section className="py-20">
        <Container>
          <h2 className="font-display text-4xl">Guides worth reading before you buy</h2>
          <div className="mt-10 grid gap-8 md:grid-cols-3">
            {guides.map((guide) => (
              <Link key={guide.slug} href={`/guides/${guide.slug}`} className="border-t border-border pt-5">
                ...
                <span className="mt-4 inline-block text-[11px] uppercase tracking-[0.16em]">Read the guide</span>
              </Link>
            ))}
          </div>
        </Container>
      ...
      <section className="border-t border-border py-16">
        <Container className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
          <div>
            <h2 className="font-display text-3xl">Visit the {site.city} showroom</h2>
